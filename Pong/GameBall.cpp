...
void GameBall::Update(float elapsedTime) {
	if (_runYet == false) {
		...
		PlayerPaddle2 *player3 = dynamic_cast<PlayerPaddle2*>(Game::GetManager().Get("Paddle3"));
		...
		if (player3 != NULL && collisionClock.getElapsedTime().asSeconds() > minCollisionInterval) {

			sf::Rect<float> p3BR = player3->GetBoundingRect();
			if (p3BR.intersects(GetBoundingRect())) {

				if (p3BR.top > GetBoundingRect().top - GetHeight() / 2)  // collision at edge
					_angle = 360.0f - _angle;
				else                                                    // collision at top
					_angle = 180.0f - _angle;
				...
				// adding spin to the ball
				float playerVelocity = player3->GetVelocity();
				// moving left
				if (playerVelocity < 0) {
					_angle -= 30.0f;
					if (_angle < 0) _angle = 360.0f - std::abs(_angle);
